...
If the repo documents an issue tracker (for example in `docs/agents/issue-tracker.md`), use it to fetch referenced issues; otherwise ask the user where issues and specs live.
...
Dispatch both subagents in a single message so they run in parallel, using a general-purpose subagent for each.
